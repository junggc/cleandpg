...
class Diagnosis extends CD_Controller {
    ...
    private function naver_payment($pay, $res, $data, $merchant) {
        ...
        if ($res['status'] == 'OK') {
            $curl = curl_init();
            $url = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? 'https://dev.apis.naver.com/naverpay-partner/naverpay/payments/recurrent/regist/v1/approval' : 'https://dev.apis.naver.com/naverpay-partner/naverpay/payments/v2.2/apply/payment';
            ...
            if ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') {
            ...
            if ($recv['code'] == "Success") {
                ...
                $res['res_result'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? $recv['body']['reserveId'] : $recv['body']['paymentId'];
                $res['res_msg'] = '성공';
                $res['status'] = 'OK';
                ...
                $resultMap['tid'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? $recv['body']['reserveId'] : $recv['body']['paymentId'];
                $resultMap['payMethod'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? '' : $recv['body']['detail']['primaryPayMeans'];
                $resultMap['payDevice'] = 'PC';
                $resultMap['vactBankName'] = '';
                $resultMap['VACT_BankCode'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? '' : $recv['body']['detail']['bankCorpCode'];
                $resultMap['CARD_BillKey'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? $recv['body']['recurrentId'] : '';
                $resultMap['VACT_Name'] = '';
                $resultMap['VACT_Num'] = '';
                ...
                $resultMap['CSHR_Type'] = '';
                $resultMap['aid'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? $recv['body']['tempReceiptId'] : $recv['body']['detail']['payHistId'];
                $resultMap['applDate'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? date('Ymd') : date('Ymd', strtotime($recv['body']['detail']['admissionYmdt']));
                $resultMap['applTime'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? date('his') : date('his', strtotime($recv['body']['detail']['admissionYmdt']));
                ...
                $resultMap['P_FN_NM'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? '' : $naverpay_card_com[$recv['body']['detail']['cardCorpCode']];
                $resultMap['CARD_Code'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? '' : $recv['body']['detail']['cardCorpCode'];
                $resultMap['CARD_Num'] = '';
    ...
    private function kakao_payment($pay, $res, $data) {
        ...
        $aPostData['cid'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? KAKAO_CID_SUBSCRIP : KAKAO_CID_EASYPAY;
        ...
        if (!isset($recv['code'])) {
            ...
            $resultMap['VACT_Num'] = '';
            $resultMap['VACT_InputName'] = '';
            $resultMap['CARD_BillKey'] = ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') ? $recv['sid'] : '';
            $resultMap['VACT_Date'] = '';
            $resultMap['ACCT_BankName'] = '';
            $resultMap['ACCT_BankCode'] = '';
            $resultMap['ACCT_Num'] = '';
        ...
        if (empty($req)) {
            ...
            if ($req['order_type'] == 'starter') {
                if ($req['pay_type'] == 'card') {
                    ...
                    /* LJB 0311 samsung 추가 */
                    $req['device_type'] == 'PC' ? 'Card' : 'CARD';
                    $val['gopaymethod'] = 'onlyssp';
                    $val['acceptmethod'] = 'cardonly';
                    $val['bank_dt'] = '';
                    $val['bank_tm'] = '';
                }
            } else {
...
//						header('Location: /diagnosis/detail?seq=' . $merchant[1]);
    ...
    public function payment_result() {
        if (empty($_REQUEST['merchantData'])) {
            ...
            } else if ($pay['order_type'] == 'subscribe' || $pay['order_type'] == 'with') {
                $pay = $this->payment_m->select_payment_temp_inicis($merchant[0]);
