<form id="SendPayForm_web" name="" method="POST">
    ...
    <input type="hidden"  name="returnUrl" value="<?php echo $base_url . RETURN_URL ?>" >
...
<form id="SendPayForm_mobile" name="SendPayForm_mobile" method="post" accept-charset="euc-kr"> 
    <!-- 리턴받는 가맹점 URL 세팅 -->
    <input type="hidden" name="P_NEXT_URL" value="<?php echo $base_url . RETURN_URL_MO ?>"> 
...
<form id="SendPayForm_mobile_bill" method="post" accept-charset="UTF-8" action="https://inilite.inicis.com/inibill/inibill_card.jsp"> 
    ...
    <input type="hidden" name="buyeremail" value=""> 
    <input type="hidden" name="buyertel" value=""> 
    <input type="hidden" name="returnurl" value="<?php echo $base_url . '/payment/payment_result_mo_bill'; ?>"> 
    <input type="hidden" name="timestamp" value="">  
            ...
            success: function (data) {
                if (data.status == 'succ') {
                    ...
                    if (data.data.gopaymethod == 'kakao') {
                        ...
                        if ($('input[name=cart_type]').val() == 'subscribe') {
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                vat_amount: 0,
                                tax_free_amount: 0,
                                approval_url: "https://dev.cleand.kr/payment/payment_result?merchantData=" + data.data.merchantData,
                                fail_url: "https://dev.cleand.kr/payment/payment_result?merchantData=" + data.data.merchantData,
                                cancel_url: "https://dev.cleand.kr/cart/cart_list?type=item"
                            },
                            success: function (data) {
                        ...
                        if ($('input[name=cart_type]').val() != 'subscribe') {
                            oPay.open({
                                ...
                                "taxExScopeAmount": "0",
                                "returnUrl": "https://dev.cleand.kr/payment/payment_result?merchantData=" + data.data.merchantData
                            });
                        } else {
                            oPay.open({
                                "actionType": "NEW",
                    ...
                    } else if (isMobile() && $('input[name=cart_type]').val() != 'subscribe') {
                        ...
                        if ($('input[name=cart_type]').val() != 'subscribe') {
                            ...
                            $.ajax({
                                ...
                                success: function () {
                                    location.href = "https://dev.cleand.kr/payment/payment_result?merchantData=" + data.data.merchantData
                                }
                            });
