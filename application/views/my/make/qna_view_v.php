            ...
            <div class="container">
                ...
                <div class="request-form">
                	<form id="frmSave">
                        ...
                        <input type="hidden" name="cqa_id" value="<?php echo $info['cqa_id']; ?>" />
                    <dl>
                        <dt>제목</dt>
                        <dd><input type="text" class="inp1" name="cqa_title" placeholder="제목을 입력해주세요" value="<?php echo $info['cqa_title']; ?>"></dd>
                    </dl>
                    <dl class="last mb50">
                        <dt>내용</dt>
                        <dd>
                            <textarea class="textarea" name="cqa_content"><?php echo $info['cqa_content']; ?></textarea>
                        </dd>
                    </dl>
                    <dl>
                        <dt>&nbsp;</dt>
                            ...
                            <div class="file-box">
								...
								?>
									<div style="margin-right:20px">
                                        ...
                                        <input type="hidden" name="file_seq[]" value="<?php echo $row['file_seq']; ?>" />
										<input type="hidden" name="orgname[]" value="<?php echo $row['org_filename']; ?>" />
										<input type="hidden" name="newname[]" value="<?php echo $row['new_filename']; ?>" />
										<input type="hidden" name="filepath[]" value="<?php echo $row['new_filepath']; ?>" />
										<input type="hidden" name="ext[]" value="<?php echo $row['file_ext']; ?>" />
										<input type="hidden" name="size[]" value="<?php echo $row['file_size']; ?>" />
										<button class="btn-del" onclick="javascript:fnDeleteFile(this);"><i class="xi-close-thin"></i></button>
                                    </div>
                                <?php
									}
								?>
                ...
                <div class="btn-box-common1">
                    ...
                    <button class="btn btn-type1 btn-m" onclick="javascript:fnSave();">수정</button>
                </div>
            </div>
        </div>
	</div>
...
{
	var del = new Array();
	var seq = $(obj).parent().find('input[name="file_seq[]"]').val();
	if(seq != '') {
		if($('input[name=deleteFile]').val() != '') {
			del = $('input[name=deleteFile]').val().split(',');
		}
		del.push(seq);

		$('input[name=deleteFile]').val(del.join(','));
	}
	$(obj).parent().remove();
}
...
function fnSave()
{
	$.ajax({
			type:'POST',
			url:'/my/make/ajaxUpdateQna',
			data : $('#frmSave').serialize(),
			dataType:"json",
			success:function(res){
				if(res.status == 'succ') {
					showAlert('info', res.msg, function() {location.href='<?php echo $move; ?>';});
				}
