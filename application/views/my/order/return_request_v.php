	...
	<div class="inner">
		...
		<div class="mypage">
			...
			<div class="container">
				<h3 class="h3"><a href="<?php echo $move; ?>" class="link m back">반품신청</a></h3>
				
				<hr class="hr pc">
				<div class="flex reverse f18 mb20">
					<div>주문일 : <?php echo $info['ins_dtm']; ?>  |  주문번호 : <?php echo $info['order_id']; ?></div>	
					<div>반품할 상품을 먼저 선택하고 수량을 입력하세요.</div>	
				</div>
						...
						?>
								<tr>
									...
									<td>1
										...
										<input type="hidden" name="unit_price[]" value="<?php echo $sum_price; ?>" />
									</td>
                            ...
                            <li>
                                <div class="item">
                                    ...
                                    <div class="info1">
                                        ...
                                        <dl>
                                            <dt>취소수량</dt>
                                            <dd><input type="tel" id="mo_req_qty_<?php echo $row['cod_id']; ?>"  name="mo_req_qty[]" a="<?php echo $row['cod_id']; ?>" class="inp1 text-center" value="0" style="width:50px" readonly></dd>
                                        </dl>
                                    </div>
                                    <div class="status">
                                        <dl>
                ...
                <div class="refund-dl mb50">
                    <dl>
                        <dt>반품 배송비</dt>
                        <dd>
                            <p id="delivery_price">0원</p>
                        </dd>
                    </dl>
                    <dl>
                        <dt>환불 금액</dt>
                        <dd><p id="refund_price">0원</p></dd>
                    </dl>
                    <dl>
                        <dt>환불은행</dt>
                        <dd>
  	                    	<select class="select1 bank_in" name="bank_code">
                    ...
                    <dl>
                        <dt>예금주</dt>
                        <dd><input type="text" class="inp1 block bank_in" name="bank_owner" placeholder="예금주를 입력하세요 " /></dd>
                    </dl>
                    <dl>
                        <dt>계좌번호</dt>
                        <dd><input type="tel" class="inp1 block bank_in" name="bank_num" placeholder="계좌번호를 입력하세요.(숫자만입력) " oninput="this.value = this.value.replace(/[^0-9]/g, '');" /></dd>
                    </dl>
                </div>
                        
				<div class="agree-box1">
					<label class="label-checkbox mb20"><input type="checkbox" name="agree" class="checkbox" ><em></em><span>반품신청 내역을 확인하였으며, 반품 신청에 동의합니다. (전자상거래법 제8조 제2항) </span></label>
                    <label class="label-checkbox"><input type="checkbox" class="checkbox" name="bankCheck"><em></em><span>환불계좌 수집이용에 동의하고, 환불 계좌 정보를 저장합니다.</span></label>
                </div>
                        
                <div class="terms1 mb60" style="padding-top:0">
                	<div>- 반품 완료시에는 무통장 입금으로 환불금액이 입금됩니다.</div>
                    <div>- 수집목적 : 취소/반품 시 환불처리</div>
                    <div>- 수집항목 : 은행, 계좌번호, 명의자 이름</div>
                         - 보유 및 이용기간 : 
                    <p>- 환불 완료 후 5년, 단 환불계좌관리에 등록할 경우 회원탈퇴 혹은 정보 삭세 시 까지</p>
                    <p>- 등록 후 환불 기록이 없을 경우 등록 1년 후 삭제</p>
                </div>
				...
				<div class="btn-box-common1">
					...
					<button class="btn btn-type1 btn-m" onclick="javascript:fnReturn();">반품신청</button>
				</div>
...
$(document).ready(function(e) {
	...
	$('select[name=reason_code]').on('change', function() {
		...
		if($(this).val() == '1') {
			delivery_price = 6000;	
			msg = '원 (반품사유  - 고객변심으로 초도배송비 + 반품배송비가 부과되었습니다. )';
		}
		$('input[name=reason]').val($('select[name=reason_code] option:selected').text());
		$('#delivery_price').html(commify(delivery_price) + msg);
		fnCalcPrice();
	});
...
function fnReturn()
{
	var msg = {msg : '반품 요청을 하시겠습니까?', cancel:'닫기', confirm : '반품'};
	showConfirm(msg, 
				function() {
						$.ajax({
							type:'POST',
							...
							success:function(res){
								if(res.status == 'succ') {
									...
									showAlert('error', res.msg, null, res.msg2);
								}
							},
							error:function(data){
